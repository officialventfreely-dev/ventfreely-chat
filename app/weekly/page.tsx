  ...
  return (
    <div className="min-h-screen bg-[#0b0614] text-white">
      ...
      <div className="mx-auto max-w-md px-5 pt-6 pb-10">
        ...
        <p className="mt-1 text-sm opacity-80">A calm summary of the last 7 days.</p>

        <div className="mt-6 rounded-2xl bg-white/10 p-5 border border-white/10">
          {loading && <div className="text-sm opacity-80">Loading your weekly report…</div>}

          {!loading && status === 401 && (
            <div>
              <div className="text-sm font-medium">Log in to see your week</div>
              <p className="mt-2 text-sm opacity-80">Your weekly report is tied to your account.</p>
              <div className="mt-4 flex gap-2">
                <Link href="/login" className="rounded-xl bg-white text-black px-4 py-2 text-sm font-medium">
                  Log in
                </Link>
                <Link href="/signup" className="rounded-xl bg-white/15 px-4 py-2 text-sm font-medium">
                  Create account
                </Link>
              </div>
            </div>
          ...
          {!loading && status === 402 && (
            <div>
              ...
              <p className="mt-2 text-sm opacity-80">Unlock Premium to view your weekly summary.</p>
              <div className="mt-4 flex gap-2">
                <a href={CHECKOUT_URL} className="rounded-xl bg-white text-black px-4 py-2 text-sm font-medium">
                  Unlock Premium
                </a>
                <Link href="/daily" className="rounded-xl bg-white/15 px-4 py-2 text-sm font-medium">
                  Open Daily
                </Link>
              </div>
            </div>
          ...
          {!loading && data && (
            <div className="space-y-4">
              <div>
                <div className="text-sm opacity-80">
                  ...
                  {data.range?.start && data.range?.end ? (
                    <span className="opacity-60"> · {data.range.start} → {data.range.end}</span>
                  ) : null}
                </div>
                ...
                <div className="mt-1 text-sm opacity-80">
                  Most felt: <span className="text-white font-medium">{data.topEmotion ?? "—"}</span>
                </div>
              </div>

              <div className="rounded-xl bg-white/10 p-4 border border-white/10">
                <div className="text-xs uppercase tracking-wide opacity-70">Last 7 days</div>
              ...
              <div className="rounded-xl bg-white/10 p-4 border border-white/10">
                <div className="text-xs uppercase tracking-wide opacity-70">Gentle suggestion</div>
                <div className="mt-2 text-sm leading-relaxed opacity-90">{suggestion}</div>
              </div>

              <div className="rounded-xl bg-white/10 p-4 border border-white/10">
                <div className="text-xs uppercase tracking-wide opacity-70">This week</div>
              ...
              <div className="pt-1 flex gap-2">
                <Link href="/daily" className="rounded-xl bg-white text-black px-4 py-2 text-sm font-medium">
                  Do today’s reflection
                </Link>
                <Link href="/chat" className="rounded-xl bg-white/15 px-4 py-2 text-sm font-medium">
                  Open chat
                </Link>
              </div>
